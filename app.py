...
# tfidf_map = [{} for _ in range(len(titles))]
# with open(os.path.join(DATA_DIR, "TFIDF.txt"), encoding="utf-8") as f:
#     for line in f:
#         i, j, val = line.strip().split()
#         tfidf_map[int(i)-1][int(j)] = float(val)
...
#     dot = sum(query_vec[int(j)] * val for j, val in doc_vec.items())
...
#     query_vec, query_norm = compute_query_vector(tokens)

#     results = []
#     for i in range(len(titles)):
#         score = cosine_similarity(query_vec, tfidf_map[i], query_norm, magnitudes[i])
#         if score > 0:
...
# # --- Load full TF-IDF matrix as dense format ---
